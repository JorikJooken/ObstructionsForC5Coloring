...
int isC5Colorable(struct graph* g, int n, int omittedVertex, int iterationCounter, int highestColorUsed)
{
    if(notColoredYetBitset==0) return 1;
    // search for vertex with fewest number of colors available
    int min_nb_colors_available=C5+5;
    int argmin=-1;
    ...
    forEach(color,availableColors[argmin])
    {
        ...
        if(isC5Colorable(g,n,omittedVertex,iterationCounter+1,newHighestColorUsed)) return 1;
...
int isC5ColorableHelper(struct graph* g, int n, int omittedVertex)
{
    if(n<=2) return 1;
    for(int i=0;i<C5;i++)
    {
        adjacentColorBitset[i]=((1LL<<(i+1)%C5)|(1LL<<(i-1+C5)%C5));
    }
    for(int i=0; i<n; i++) availableColors[i]=(1LL<<C5)-1;
    ...
    int ret=isC5Colorable(g,n,omittedVertex,1,0);
    return ret;
}

// generates all C5-colorings
// this function should only be called from with the function "generateAllC5ColoringsHelper"
void generateAllC5Colorings(struct graph* g, int n, bitset omittedVertices, int iterationCounter, int calcPoorVertex, int highestColorUsed)
{
    ...
    forEach(color,availableColors[argmin])
    {
        ...
        int newHighestColorUsed=highestColorUsed;
        if(color>highestColorUsed) newHighestColorUsed=color;
        generateAllC5Colorings(g,n,omittedVertices,iterationCounter+1,calcPoorVertex,newHighestColorUsed);
        // restore neighbor availabilities
        forEach(neigh,(g->adjacencyList[argmin]&notColoredYetBitset))
        {
            availableColors[neigh]=oldAvailableColorsAtIteration[iterationCounter][neigh];
...
// generates all C5-colorings
// this is the function that should be called by the user.
void generateAllC5ColoringsHelper(struct graph* g, int n, bitset omittedVertices, int calcPoorVertex)
{
    for(int i=0;i<C5;i++)
    {
        adjacentColorBitset[i]=((1LL<<(i+1)%C5)|(1LL<<(i-1+C5)%C5));
    ...
    return generateAllC5Colorings(g,n,omittedVertices,2,calcPoorVertex,0);
}
...
void compute_hull(struct graph* g, int currVertex, bitset omittedVertices)
{
    ...
    generateAllC5ColoringsHelper(g,currVertex,omittedVertices,0);
}
...
void addEdgesInAllPossibleWays(struct graph* g,  struct similarGraphsList* sgl, int currVertex, int numberOfVertices, int *ctr, struct options *options, int otherEndPointAfter, bitset *validEndPoints, int numberSimilarVertices, struct counters *counters, int poor_vertex, int max_palette_size_G_minus_u)
{
    ...
    if(!has_path && !has_F && !isC5ColorableHelper(g,currVertex+1,123)) // graph cannot be colored
    {
        ...
        {
            if(!isC5ColorableHelper(g,currVertex+1,omit)) allInducedSubgraphsColorable=false;
        }
    }
    ...
    if(poor_vertex<0)
    {
        ...
        if(structureBroken1 && !isolated && graphColorable && ! has_F && !has_path)
        {
            if(numberSimilarVertices>=1)
            {
                ...
                compute_hull(g,currVertex+1,omittedVertices);
                int someOk=0;
    ...
    if(poor_vertex>=0 && !isolated && graphColorable && ! has_F && !has_path)
    {
        ...
        generateAllC5ColoringsHelper(g,currVertex+1,omittedVertices,1);
...
bool canBeExtended(struct graph* g, struct similarGraphsList* sgl, int currVertex, int subgraphOrder, int subgraphCurrentVertex, bitset verticesUsed, bool useHull)
{
    if(subgraphCurrentVertex==subgraphOrder)
    {
        ...
        if(useHull)
        {
            compute_hull(g,currVertex,V1AsBitset);
        }
        else
        {
            for(int i=0; i<currVertex; i++)
    ...
    forEach(i,compl(verticesUsed,currVertex))
    {
        ...
        forEach(j,compl((verticesUsed|(1LL<<i)),currVertex))
        {
            ...
            if(edgesAreSubset)
            {
                ...
                bool can=canBeExtended(g,sgl,currVertex,subgraphOrder,subgraphCurrentVertex+1,(verticesUsed|(1LL<<i)|(1LL<<j)),useHull);
                if(can) return true;
            }
        }
    }
...
bool findSimilarGraphs(struct graph* g, struct similarGraphsList* sgl, int currVertex, int subgraphOrder, bool useHull)
{
    for(int i=0; i<currVertex; i++)
    {
        for(int j=0; j<currVertex; j++)
        {
            ...
            graph1[0]=i;
            graph2[0]=j;
            bool can=canBeExtended(g,sgl,currVertex,subgraphOrder,1,verticesUsed,useHull);
            if(can) return true;
        }
    }
    return false;
...
void addVertex(struct graph* g, struct similarGraphsList* sgl, int currVertex, int numberOfVertices, int *ctr, struct options *options, struct counters *counters)
{
    ...
    if(sm!=-1)
    {
        ...
        /*if(currVertex>=13)
        {
            fprintf(stderr,"sm and la: %d %d\n",sm,la);
            writeToG6(g->nautyGraph,currVertex);
            printNautyGraph(g->nautyGraph,currVertex);
        }*/
        ...
        validEndPoints=intersection(validEndPoints,compl(g->adjacencyList[sm],currVertex)); // triangle-free
    ...
    if(currVertex<=hullConstant) // only compute when the current graph is small enough
    {
        ...
        for(int smallestVertex=currVertex-1; smallestVertex>=0 && sm==-1; smallestVertex--)
        {
            ...
            compute_hull(g, currVertex, omittedVertices);
    ...
    int oldSglSize=sgl->size;
    bool similarEdgesFound=findSimilarGraphs(g,sgl,currVertex,subgraphOrder,false);
    ...
    {
        oldSglSize=sgl->size;
        similarEdgesFound=findSimilarGraphs(g,sgl,currVertex,subgraphOrder,true);
        if(similarEdgesFound) // similar edges were found
        {
    ...
    oldSglSize=sgl->size;
    bool similarTriangleFound=findSimilarGraphs(g,sgl,currVertex,subgraphOrder,false);
    ...
    /*oldSglSize=sgl->size;
    similarTriangleFound=findSimilarGraphs(g,sgl,currVertex,subgraphOrder,true);
    if(similarTriangleFound) // similar triangle was found
    {
        //fprintf(stderr,"Entered!\n");
        counters->nOfTimesSimilarTriangle[currVertex]++;
    ...
    oldSglSize=sgl->size;
    bool similarP3Found=findSimilarGraphs(g,sgl,currVertex,subgraphOrder,false);
    ...
    oldSglSize=sgl->size;
    bool similarDiamondFound=findSimilarGraphs(g,sgl,currVertex,subgraphOrder,false);
    ...
    /*oldSglSize=sgl->size;
    similarDiamondFound=findSimilarGraphs(g,sgl,currVertex,subgraphOrder,true);
    if(similarDiamondFound) // similar diamond was found
    {
        counters->nOfTimesSimilarDiamond[currVertex]++;
        addEdgesInAllPossibleWays(g,sgl,currVertex,numberOfVertices,ctr,options,-1,&validEndPoints,subgraphOrder,counters,-1,-1);
    ...
    oldSglSize=sgl->size;
    bool similarP4Found=findSimilarGraphs(g,sgl,currVertex,subgraphOrder,false);
    ...
    oldSglSize=sgl->size;
    bool similarC4Found=findSimilarGraphs(g,sgl,currVertex,subgraphOrder,false);
    ...
    subgraphAdjacencyMatrix[3][0]=1;
    oldSglSize=sgl->size;
    bool similarK13Found=findSimilarGraphs(g,sgl,currVertex,subgraphOrder,false);
    ...
    oldSglSize=sgl->size;
    bool similarK4Found=findSimilarGraphs(g,sgl,currVertex,subgraphOrder,false);
    ...
    oldSglSize=sgl->size;
    bool similarComplP1PlusP3Found=findSimilarGraphs(g,sgl,currVertex,subgraphOrder,false);
    ...
    /*if(currVertex>=13)
        {
            fprintf(stderr,"No similarity lemma was used\n");
            writeToG6(g->nautyGraph,currVertex);
            printNautyGraph(g->nautyGraph,currVertex);
        }*/
...
int main(int argc, char ** argv) {
    ...
    fprintf(stderr,"Results for vertex-critical (P%d,F)-free graphs on %d vertices for C5-coloring\n",options.pathLength,numberOfVertices);
